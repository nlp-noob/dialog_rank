...
class Trainer:
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        data_collator = self.data_collator
        group_id_list = eval_dataset["group_id"]
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        """
        data_collator = self.data_collator
        group_id_list = test_dataset["group_id"]
    ...
    def evaluate(
        ...
        eval_dataloader, group_id_list  = self.get_eval_dataloader(eval_dataset)
        start_time = time.time()
    ...
    def predict(
        ...
        test_dataloader, group_id_list = self.get_test_dataloader(test_dataset)
        start_time = time.time()
        ...
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            if args.include_inputs_for_metrics:
                metrics = self.compute_metrics(
                    EvalPrediction(predictions=all_preds, label_ids=all_labels, inputs=all_inputs), group_id_list
                )
            else:
                metrics = self.compute_metrics(EvalPrediction(predictions=all_preds, label_ids=all_labels), group_id_list)
        else:
            metrics = {}
    ...
    def prediction_loop(
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            if args.include_inputs_for_metrics:
                ...
                metrics = self.compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids), group_id_list)
        else:
            metrics = {}
