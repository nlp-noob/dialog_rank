...
from utils.rank_metrics import RankEvaluator
...
def get_my_dataset(customer_args):
    ...
    dataset_dict["train"] = Dataset.from_pandas(pd.DataFrame(train_data_json))
    dataset_dict["validation"] = Dataset.from_pandas(pd.DataFrame(validation_data_json))
...
def main():
    ...
    jf = open(customer_args.label_info_path, "r")
    label_info = json.load(jf)
    jf.close()
    ...
    def my_metrics(pred_labels, true_labels, probs, group_id_list):        
        id2label = {0: "negative", 1: "positive"}

        top_k_eval_list = [1, 2, 3, 4, 5]
        evaluator = RankEvaluator()
        pred_labels_bias = (pred_labels - 0.5) * 2
        max_scores = np.max(probs, axis=1)
        scores = max_scores * pred_labels_bias
        map_acc = evaluator.get_map(true_labels, scores)
        top_k_acc_list = evaluator.get_top_k_acc_list(true_labels, scores, top_k_eval_list, group_id_list)

        # init metrics_dict for all
        metric_dict = {"TP": 1e-7, "FP":1e-7, "FN":1e-7} # avoid devide zero error 
        metric_dict["map"] = map_acc
        for top_k_to_eval, top_k_acc in zip(top_k_eval_list, top_k_acc_list):
            metric_dict["top_{}".format(top_k_to_eval)] = top_k_acc

        for a_pred, a_true in zip(pred_labels, true_labels):
            if a_pred == 1 and a_true == 1:
                metric_dict["TP"] += 1
            elif a_pred == 1 and a_true == 0:
                metric_dict["FP"] += 1
            elif a_pred == 0 and a_true == 1:
                metric_dict["FN"] += 1
        ...
        # PR AUC
        # auc = my_PR_AUC(true_labels, probs)
        # metric_dict["PR_AUC"] = auc
    ...
    def compute_metrics(p: EvalPrediction, group_id_list: list):
        id2label_tb = {0: "negative", 1: "positive", 2:"over_all"}
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.squeeze(preds) if is_regression else np.argmax(preds, axis=1)
        if data_args.task_name is not None:
            result = metric.compute(predictions=preds, references=p.label_ids)
            # compute my metrics here
            my_result = my_metrics(preds, p.label_ids, p.predictions, group_id_list)
            print(my_result)
