...
class MLMBiencoder(nn.Module):
    ...
    def forward(self, input_mlm, cont, resp, cont_tri, p_resp, n_resp, label):
        if self.mlm:
        ...
        if cont_tri is not None:
            ...
            cont_tri = cont_tri.to(self.device)
            p_resp = p_resp.to(self.device)
            n_resp = n_resp.to(self.device)
            _, hid_cont_tri = self.encoder_forward(
                    input_ids       = cont_tri,
                    attention_mask  = cont_tri > 0,
            )
            ...
            hid_cont_tri = torch.nn.functional.normalize(hid_cont_tri, p=2, dim=1)
            ...
            distance_positive = (hid_cont_tri - hid_p_resp).pow(2).sum(1) 
            distance_negative = (hid_cont_tri - hid_n_resp).pow(2).sum(1) 
            triplet_loss = nn.TripletMarginLoss(margin=self.margin, p=2)
            loss_tri = triplet_loss(hid_cont_tri, hid_p_resp, hid_n_resp)
        else:
            loss_tri = torch.tensor(0)
            distance_positive = torch.tensor(0)
            distance_negative = torch.tensor(0)
        ...
        return loss, (loss_mlm, loss_tri, loss_rs, scores, distance_positive, distance_negative, hid_cont, hid_resp)
